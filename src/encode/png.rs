...
#[wasm_bindgen]
pub fn encode_png(pixels: &[u8], width: u32, height: u32, channel: u32) -> Vec<u8> {
    let mut v = vec![];
    let encoder = image::codecs::png::PngEncoder::new(&mut v);
...
#[wasm_bindgen]
pub fn decode_png(pixels: &[u8]) -> Image {
    let decoder = image::codecs::png::PngDecoder::new(Cursor::new(pixels)).unwrap();
